...
void set_destroy(Set set)
{
	...
	node_destroy(set->temp_root, set->destroy_value);
	free(set);
}
...
void set_visit(Set set, VisitFunc visit)
{
	if (set->root == NULL)
		return;
	else if (set->begin == false)
	{
		set->temp_root = set->root;
		set->begin = true;
	}

	visit(set->temp_root->value);

	if (set->temp_root->left != NULL)
		visit(set->temp_root->left->value);

	if (set->temp_root->right != NULL)
		visit(set->temp_root->right->value);

	set->temp_root = set_next(set, set->temp_root);
	set->temp_root = set_next(set, set->temp_root);

	if (set->temp_root == NULL)
		return;
