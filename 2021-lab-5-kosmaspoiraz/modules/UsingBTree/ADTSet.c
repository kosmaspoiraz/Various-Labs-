...
static SetNode node_find_previous(SetNode set_node, CompareFunc compare)
{
	...
	if (index == 0)
	{ // Ο set_node είναι πρώτος μέσα στο btree node
...
static SetNode node_find_next(SetNode set_node, CompareFunc compare)
{
	...
	if (index == btree_node->count - 1)
	{ // Ο set_node είναι τελευταίος μέσα στο btree node
...
void recursive_visit(BTreeNode node, VisitFunc visit)
{
	int i;
	if (node)
	{
		for (i = 0; i < node->count; i++)
		{
			recursive_visit(node->children[i], visit);
...
void set_visit(Set set, VisitFunc visit)
{
	...
	recursive_visit(set->root, visit);
}